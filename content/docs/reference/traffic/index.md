...
mirrord's outgoing traffic feature intercepts outgoing requests (in outgoing connections) from the local process and 
...
cluster, for example, a database read/write. Note that in such an example, if a user request triggers an outgoing API
call over the network, and [traffic stealing](#stealing) is not enabled, this could lead to two calls being sent out
of the remote pod for one user request - one API call generated by the remote process, and one generated by the local
process (and emitted out of the remote pod with the help of mirrord).
Outgoing UDP is currently only intercepted and forwarded by mirrord if the application binds a non-0 port and makes
a `connect` call on the socket before sending out messages. Outgoing TCP and UDP forwarding are both enabled by
default. It can be controlled individually for TCP and UDP or disabled altogether (see `mirrord exec
--help`).
