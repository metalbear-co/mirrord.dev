...
1. Statically linked binaries. Since mirrord uses the dynamic linker to load into the application's process,
   ...
   your program code. If you don't want to add an import to your Go program, you can alternatively build a dynamically linked binary using `go build -ldflags='-linkmode external'`.
   Using `go run` is not possible at the moment - please follow [this issue](https://github.com/metalbear-co/mirrord/issues/1922) for updates.   
