...
2. If your local process reads from a queue, you might want to test out the [copy target feature](/docs/using-mirrord/copy-target/), which temporarily creates a copy of the, allowing you to use the `scaledown` flag to be the only copy so none competes with your local process for queue messages.
